...
"""
Update log:
12/01/2024: dynamic P cycle added. after Komar and Zeebe 2017.
12/01/2024: effect of saturation state on rain ratio added
12/01/2024: dynamic nuwd added
...
def parameters(exp_name = 'test', LOADFLAG = 0, RUNTYPE = 1, pcycle = 1, FTYS = 0, FSED = 1, tsnsflag=1, svstart = 0, svresults = 1, printflag = 1,
               t0 = 0, tfinal = 2e6, pref = 280, pcsi = 280, fepl = 0.8, frei = 0.78, eph = 1.8, rrain = 6.1, fsh = 1,
               fsi0 = 5, finc0 = 12, d13cin = 1.5, d13cvc = -4, thc = 20, cac = 0.0103, mgc = 0.053, sclim = 3, nsi = 0.2, ncc = 0.4,
               cinpflag = 0,  initfile = 'preind_steady.dat', cinp = 3000, dccinp = -55, tcin0 = 0, tcspan = 6e3, cinpfile = 'emi_file.dat',
               svfile = 'preind_steady.dat', target = 'GSpH', target_file = 'ph.csv', target_file2 = 'd13c.csv', toms_low = 0, toms_high = 3,
               exp_nuwd = 0, exp_rrain = 0, scale_conveyor = 0, hsl = 0.08, kasv_const = 0.06, id = 0
               ):
    params = {
        'exp_name': exp_name,
        'LOADFLAG': LOADFLAG,
        'RUN_TYPE': RUNTYPE,              # 1: forward, 2: inverse
        'pcycle': pcycle,
        'FTYS': FTYS,
        'FSED': FSED,
        'tsnsflag': tsnsflag,
        ...
        'exp_nuwd': exp_nuwd,
        'exp_rrain': exp_rrain,
        'scale_conveyor': scale_conveyor,
        'hsl': hsl,
        'kasv_const': kasv_const
...
def init_start(params):
    ...
    global exp_nuwd, exp_rrain, scale_conveyor, pcycle
    exp_nuwd = params['exp_nuwd']
    exp_rrain = params['exp_rrain']
    scale_conveyor = params['scale_conveyor']
    ...
    global frei, hsl, kasv_const
    frei = params['frei']          # fraction EPL, remineralized in I boxes
    hsl = params['hsl']
    kasv_const = params['kasv_const']
    ...
    # table 2
    global t0, tfinal, pref, pcsi, fepl, eph0, rrain0, fsh, fvc0, finc0, d13cin, d13cvc, thc0, cac, mgc, sclim, nsi, ncc, fsi0
    t0 = params['t0']
    ...
    if cinpflag == 2 or cinpflag == 3:
        ...
        try:
            ...
            print("Please provide the right file name for the argument emission_file. ")
            print("The data in the provided file should be two/three columns in shape without a header line")
...
def model_run():
    ...
    if RUN_TYPE == 1:
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], method = solver)

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], method = solver, first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls], method = solver, first_step = 1e2)



        else:
            first_step = None
            ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], method = solver, first_step = first_step)
        ...
        # print(f'{elapse: .2f}s used.')
            ...
            ph, ccda, ccdp, pco2,co3, temp1, temp2 = ph_ccd_cal(ysol['y'], ysol['t'])

            global ph0, ccda0, ccdp0, surf_d13c0, co30, co3_surf0
            ph0 = ph[-1]
            ccda0 = ccda[-1]
            ccdp0 = ccdp[-1]
            co30 = co3[kdv,-1]
            co3_surf0 = co3[m1,-1]
            dicc = yfinal[5*NB:6*NB] * CNTI
            dic = yfinal[0:1*NB]
            ...
            surf_d13c0 = np.sum(d13c_mol[kkv] * vb[kkv])/np.sum(vb[kkv])
            return yfinal[NOCT*NB] / PPMTOMMSQ/CNTI, ph, ccda, ccdp
    ...
    if RUN_TYPE == 2:
        ...
        if tracer_type == 'pCO2':
            ...
            for i in range(len(t_target)-1):
                if i:
                    y0 = np.loadtxt(f'inv_ystart_{ith_loop}.dat')
                    y0[3*NB:4*NB] /= TSCAL
                else:
                    y0 = ystart
                ...
                if platform.system() == 'Windows':
                    ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]],args = [ hpls],  method = solver)

                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]],args = [hpls],  method = solver, first_step = 5e2)


                        if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                            ysol =  solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]],args = [ hpls],  method = solver, first_step = 1e2)



                else:
                    first_step = None
                    ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]],args = [ hpls],  method = solver,  first_step = first_step)



                yfinal = ysol['y'][:,-1]
                yfinal[3*NB:4*NB] *= TSCAL
                np.savetxt(f'inv_ystart_{ith_loop}.dat', yfinal)
            ...
            # emi_scenario.to_csv(join(dir_name, f'inverse_emission_from_pco2_{fsi0/1e12 :.1f}_{finc0*AOC/1e12 :.1f}.csv'))




            if platform.system() == 'Windows':
                ysol = solve_ivp(derivs, (t_target[0]-500, t_target[-1]), ystart, args =  [hpls], method = solver)

                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =solve_ivp(derivs, (t_target[0]-500, t_target[-1]), ystart, args = [ hpls],  method = solver, first_step = 5e2)


                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =  solve_ivp(derivs, (t_target[0]-500, t_target[-1]), ystart, args = [ hpls],  method = solver, first_step = 1e2)



            else:
                first_step = None
                ysol = solve_ivp(derivs, (t_target[0]-500, t_target[-1]), ystart, args = [ hpls], method = solver, first_step = first_step)




            # print(f'{elapse: .2f}s used.')
            # print(f'The calculated emission scenario has been saved to inverse_emission_from_pco2_{fsi0/1e12 :.1f}_{finc0*AOC/1e12 :.1f}.csv')
            ...
            os.remove(f'inv_ystart_{ith_loop}.dat')
            return emi_c_cum[-1], ysol['t'], ysol['y']
            ...
            for i in range(len(t_target)-1):
                ...
                if platform.system() == 'Windows':
                    ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, args = [ hpls], t_eval = [tems[1]], method = solver)

                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =solve_ivp(derivs, (tems[0], tems[1]), y0, args = [ hpls], t_eval = [tems[1]],  method = solver, first_step = 5e2)


                        if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                            ysol =  solve_ivp(derivs, (tems[0], tems[1]), y0, args = [ hpls], t_eval = [tems[1]], method = solver, first_step = 1e2)



                else:
                    first_step = None
                    ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, args = [ hpls], t_eval = [tems[1]], method = solver, first_step = first_step)
            ...
            if platform.system() == 'Windows':
                ysol = solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [ hpls], method = solver)

                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [ hpls],   method = solver, first_step = 5e2)


                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =  solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [ hpls],  method = solver, first_step = 1e2)



            else:
                first_step = None
                ysol = solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [ hpls],  method = solver, first_step = first_step)
            ...
            emi_scenario = pd.DataFrame(np.vstack([tt, emi_c, emi_c_cum]).T)
            # emi_scenario.columns = ['Age', 'Carbon_emission_Gt', 'total_carbon_emission_Gt']
            # emi_scenario.to_csv(f'inverse_emission_from_ph_{ith_loop}.csv')
            ...
            if svresults:
                print('Starting to save the modeling results.')
                omegaclc, omegaarg, surface_sir, high_omegaclc, high_omegaarg, init_omegaclc, init_omegaarg, init_high_omegaclc, init_high_omegaarg, init_sir = wo_results(ysol['y'], ysol['t'])
                print('Results are saved')

                emi_scenario.columns = ['Age', 'Carbon_emission_Gt', 'total_carbon_emission_Gt']
                emi_scenario.to_csv(f'{exp_name}/inverse_emission_from_ph_{ith_loop}.csv')

                print('The calculated emission scenario has been saved to inverse_emission_from_ph.csv')


            os.remove(f'inv_ystart_{ith_loop}.dat')
            return emi_c_cum[-1], omegaclc, omegaarg, surface_sir, high_omegaclc, high_omegaarg, init_omegaclc, init_omegaarg, init_high_omegaclc, init_high_omegaarg, init_sir
...
#
            ...
            for i in range(len(t_target_d13c)-1):
                ...
                if platform.system() == 'Windows':
                    ysol = solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [ hpls], t_eval = [tems_d13c[1]], method = solver)

                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [ hpls], t_eval = [tems_d13c[1]], method = solver, first_step = 5e2)


                        if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                            ysol =  solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [hpls], t_eval = [tems_d13c[1]], method = solver, first_step = 1e2)



                else:
                    first_step = None
                    ysol = solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [ hpls], t_eval = [tems_d13c[1]], method = solver, first_step = first_step)
            ...
            if platform.system() == 'Windows':
                ysol = solve_ivp(derivs, (t_start-500, t_end), ystart, args = [ hpls], method = solver)

                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =solve_ivp(derivs, (t_start-500, t_end), ystart, args = [ hpls], method = solver, first_step = 5e2)


                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =  solve_ivp(derivs, (t_start-500, t_end), ystart, args = [ hpls], method = solver, first_step = 1e2)



            else:
                first_step = None
                ysol = solve_ivp(derivs, (t_start-500, t_end), ystart, args = [ hpls], method = solver, first_step = first_step)

            print(f'{elapse: .2f}s used.')
            print('The calculated emission scenario has been saved to double_inversion_emission.csv and double_inversion_emission_d13c.csv')
            ...
            print(f'{elapse: .2f}s used.')
            print('The calculated emission scenario has been saved to double_inversion_emission_pH.csv and double_inversion_emission_d13c_pH.csv')
...
def cost_function(ems_new, tracer_type, hpls, tems, ems):
    ...
    if tracer_type == 'GSpH':
        ...
        surface_ph = -np.log10(np.sum(np.transpose(h_concentration)[:, kkv] * vb[kkv],axis = 1)/np.sum(vb[kkv]))


        errors = surface_ph0 - surface_ph  - f_target(tems[-1])
...
def wo_results(y, t):
    ...
    surface_ph = -np.log10(np.sum(np.transpose(h_concentration)[:, kkv] * vb[kkv],axis = 1)/np.sum(vb[kkv]))
    surface_omegaclc =  np.sum(np.transpose(co3/1e6*cac/kspc)[:, m1] * vb[m1],axis = 1)/np.sum(vb[m1])
    surface_omegaarg =  np.sum(np.transpose(co3/1e6*cac/kspa)[:, m1] * vb[m1],axis = 1)/np.sum(vb[m1])
    surface_co3 = np.sum(np.transpose(co3/1e6)[:, kkv] * vb[kkv],axis = 1)/np.sum(vb[kkv])
    surface_sir = 10**-surface_ph/surface_co3

    high_omegaclc = np.transpose(co3/1e6*cac/kspc)[:, 9]
    high_omegaarg = np.transpose(co3/1e6*cac/kspa)[:, 9]
    ...
    pd_surface = pd.DataFrame(
        {'surface_dic': surface_dic,
         ...
         'surface_omegaclc': np.transpose(surface_omegaclc),
         'surface_omegaarg': np.transpose(surface_omegaarg),
         'high_omegaclc': np.transpose(high_omegaclc),
         'high_omegaarg': np.transpose(high_omegaarg),
         'surface_sir': np.transpose(surface_sir)*1e5

            },
        index = t
        )
    pd_surface.index.name = 'Age'
    pd_surface.to_csv(join(dir_name, "Surface_dic_alk_d13c_ph.csv"), float_format = '%.15f')
    ...
    return np.min(surface_omegaclc), np.min(surface_omegaarg), np.max(surface_sir)*1E5, np.min(high_omegaclc), np.min(high_omegaarg), surface_omegaclc[0], surface_omegaarg[0], high_omegaclc[0], high_omegaarg[0], surface_sir[0]*1e5


def ph_ccd_cal(ysol, t):
    dic = ysol[0:NB,:]
    ...
    # calculate surface supersaturation
    salv0 = salv[0] * np.ones(len(t))
    prsv0 = prsv[0] * np.ones(len(t))
    if len(tcb_surf) >1:
        kspc = fkspc(tcb_surf.flatten(), salv0.flatten(), prsv0.flatten(), cac, mgc)
        kspa = fkspa(tcb_surf.flatten(), salv0.flatten(), prsv0.flatten(), cac, mgc)
    else:
        kspc = fkspc(tcb_surf, salv0, prsv0, cac, mgc)
        kspa = fkspa(tcb_surf, salv0, prsv0, cac, mgc)

    co3_surf = np.array(co3).reshape(NB, len(t))
    co3_surf = np.sum(np.transpose(co3_surf)[:, m1] * vb[m1],axis = 1)/np.sum(vb[m1])
    kspc = np.array(kspc).reshape(1, len(t))
    kspa = np.array(kspa).reshape(1, len(t))

    omegaclc = np.transpose(co3_surf*cac/kspc)
    omegaarg = np.transpose(co3_surf*cac/kspa)
    ...
    return surface_ph, np.argmin(faca, axis = 0), np.argmin(fpca, axis = 0), pco2a, np.array(co3).reshape(NB, len(t)), omegaclc, omegaarg
...
def model_initialize():
    global ab, alpcorg, asva, asvi, asvp, cac, dsv, epscorg, fc0a, fc0i, fc0p, fcc0a, fcc0i, fcc0p, fconv, fcsml, fdapi, fkrg0, frei, frrf, fsh, \
            gp, gtha, gthi, hb, hsl, jsh, kiv, kkv, kdv, klid, kssd, m1, m2, m3, mgc, mhd0, mxv0, ncc, ncsd, nlid, nsi, nuwd0, nz, oi, pcsi, phi0, phi1, phiia, phiii, phiip, prsv, \
                rincc, rkrgcc, rvccc, salv, tcb0, tcb0c, thbra, thbri, thc_i, tso, tto, vb, ystart, zv
    ...
    if FTYS:
        fconv = 3
        thc_i = thc0 * 1e6 * YTOSEC  # m3/y Conveyor transport
        tto = 2e6 * YTOSEC   # m3/y Tethys Transport
    else:
        fconv = 1
        thc_i =thc0 * 1e6 * YTOSEC
        tto = 0
    ...
    if FTYS:
        ...
        kdv =  np.array([6,7,8,12], dtype = 'int8')
    else:
        kiv = np.array([3,4,5], dtype = 'int8')
        kdv =  np.array([6,7,8], dtype = 'int8')
    ...
    """============        mixing parameters          ==========="""
    mxv0 = 3.8 * YTOSEC * mv0         #Sv -> m3/year  3.8: tuning
    mhd0 = 1.3 * YTOSEC * mhd0        #Sv -> m3/year  1.3: tuning
    ...
    if FSED:
        nuwd0 = 0.31      # water column dissolution
    else:
         nuwd0 = 0.0
    ...
    if FTYS:
        fkrg0 = 7e12/AOC    # mol C/m2/y 7 [WK92, Berner]
...
# initial porosities and CaCO3 mass
    ...
    if pcycle:
        ...
        fcexp0 = np.sum(fepl * mxv0[0:NLS] * po4[kiv]/REDPC) + eph0
        fpexp0 = np.sum(fepl * mxv0[0:NLS] * po4[kiv]) + eph0 * REDPC
        ...
        capk = fcap0 / (np.sum(fepl * mxv0[0:NLS] * po4[kiv]) * (1-po4bf0)/AOC ) #  rate const. for Ca sorbed P burial
    ...
    if LOADFLAG:
        ...
        if RUN_TYPE == 2:
            ...
            global co30, co3_surf0, surface_ph0
            co3, omega0, sir0, surface_ph0, omega_m10 = derivs(0, ystart, hpls, 1)
...
def derivs(t, y,  hpls, *flux_flag ):
    ...
    if RUN_TYPE == 2 or RUN_TYPE == 3 :
        ftm = scale_conveyor * (tcb[0]-tcb0[0])/tcb0[0]
    ...
    kspc = fkspc(tcb, salv, prsv, cac, mgc)
    kspa = fkspa(tcb, salv, prsv, cac, mgc)
    hco3  = dic/1e3 - co2 - co3

    # air-sea co2 exchange coeff

    kasv0[kkv] = kasv_const
    vask0[kkv] = 3*365
    kasv = kasv0 * ab
    vask = vask0 * ab
    ...
    if RUN_TYPE == 2 or RUN_TYPE == 3:
            ...
            rrain = rrain0 / (((*cac*co3/kspc)[m1]-1)/(omega0[m1]-1))**exp_rrain)
    ...
    if RUN_TYPE == 2 or RUN_TYPE == 3:
        ...
        nuwd = 1-(1-nuwd0) * (co3[kdv]/co30) ** exp_nuwd

        nuwd[nuwd<0.1] = 0.1
        nuwd[nuwd>0.9] = 0.9

    else:
        nuwd = np.ones(len(m3)) * nuwd0
    ...
    if FSED:
        ...
        rscva, rscvacc, rdva, rdvacc, wcva, wcvacc, gsda, dissa, dissmcca, fba =  funcsed(ealv[0], ealvcc[0], eah, eahcc, ab[0], fccva, fcva, fc0a, asva, phiia, tcb, co3, gp[6], 1, 0, fsh, nuwd[0])
        rscvi, rscvicc, rdvi, rdvicc, wcvi, wcvicc, gsdi, dissi, dissmcci, fbi =  funcsed(ealv[1], ealvcc[1], eah, eahcc, ab[1], fccvi, fcvi, fc0i, asvi, phiii, tcb, co3, gp[7], 1, 1, fsh, nuwd[1])
        rscvp, rscvpcc, rdvp, rdvpcc, wcvp, wcvpcc, gsdp, dissp, dissmccp, fbp =  funcsed(ealv[2], ealvcc[2], eah, eahcc, ab[2], fccvp, fcvp, fc0p, asvp, phiip, tcb, co3, gp[8], 0.5, 2, fsh, nuwd[2])


        if FTYS:
            rscvt, rscvtcc, rdvt, rdvtcc, wcvt, wcvtcc, gsdt, disst, dissmcct, fbt =  funcsedt(ealv[3], ealvcc[3], eah, eahcc, ab[10], fccvt, fcvt, fc0t, asvt, phiit, tcb, co3, 0, 6, fsht, nuwd[3])
    ...
    dicp[6:9] = dicp[6:9] + exh/vb[6:9]*gp[6:9] + 0.5*nuwd[0:3]*eah/vb[6:9]*gp[6:9]
    ...
    alkp[6:9] += (sdn*eah/vb[6:9] - (frei + oip)*enh/vb[6:9])*gp[6:9] + nuwd[0:3]*eah/vb[6:9] * gp[6:9]
   ...
   # =========================== Oxygen ===================================
    ...
    if NOCT >= 6:
        ...
        diccp[6:9] += exhcc/vb[6:9] * gp[6:9] + 0.5*nuwd[0:3]*eahcc/vb[6:9] * gp[6:9]
    ...
    if not flux_flag:
        ...
        h_concentration = 10 ** -ph
        surface_ph = -np.log10(np.sum(h_concentration[kkv] * vb[kkv])/np.sum(vb[kkv]))

        return co3, co3*cac/kspc, hco3/(10**-ph)/1e6, surface_ph
...
def funcsed(ealv, ealvcc, eah, eahcc, ab, fccv, fcv, fc0, asv, phii, tcb, co3,gp, coef, i, fsh, nuwd):
    # input
    # ealv: caco3 export * 2
...
@jit(nopython = True)
# @jit(nopython = True , nogil = True)
def funcsedt(ealv, ealvcc, eah, eahcc, ab, fccv, fcv, fc0, asv, phii, tcb, co3, gp, coef, fsht, nuwd):
    # input
    # ealv: caco3 export * 2
    # ealvcc: d13 of caco3 export
    # eah: high-latitude caco3 input, 0
